...
l0 = new loc ({
    text:"Добро пожаловать в Совиные грёзы<audio src='files/prolog.mp3' autoplay='autoplay' loop>",
    name:"Пролог",
    fon:"img/prolog.png"
})
l1 = new loc ({
...
text.onclick = function(){
    ...
    switch (way){
        ...
        //Пролог - добавлен после отзывов - Асасин Юлий прибывает к королевской башне за работой, он видит Боба который переносит ящики с продуктами из повозки на кухню и решает ему помочь, в ходе пролога Юлий помогает бобу с продуктами, создаёт рецепт салата цезарь, договаривается о том что будет доставлять бобу ингредиенты для этого салата, по пути из башни видит как ГГ пьяный в хлам пробирается на территорию башни.
        ...
        //Пролог
        add(()=>{
            p6.rename("Юлий (Мысли)")
            p6.render()
            print("(Так, я наконец то прибыл к королевской башне)")
            l0.render()
        })
        add(()=>{
            print("(Мне давно хотелось попасть сюда)")
        })
        add(()=>{
            p6.rename("Юлий")
            print("Мистер, вам помочь?")
        })
        //конец пролога
        add(()=>{
            p1.render()
